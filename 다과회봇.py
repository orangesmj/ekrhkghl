...
async def open_bundle(interaction: discord.Interaction, item: str, amount: int):
    ...
    # 보상 획득 로직 (확률 적용)
    if item == "쿠키꾸러미(소)":
        reward_options = [(3, 0.7), (5, 0.3)]
    elif item == "쿠키꾸러미(중)":
        reward_options = [(7, 0.6), (10, 0.4)]
    else:  # 쿠키꾸러미(대)
        reward_options = [(15, 0.5), (30, 0.5)]

    # 보상 지급 수량 결정
    choices, weights = zip(*reward_options)
    base_rewards = random.choices(choices, weights=weights, k=amount)
    total_reward = int(sum(base_rewards) * (1.5 if coffee_active else 1))
